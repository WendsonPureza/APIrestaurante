...
module.exports = (db) => {
  ...
  router.get("/periodo", (req, res) => {
    ...
    db.all(sql, [data_inicio, data_fim], (err, rows) => {
      if (err) {
        console.error("Erro ao gerar relatório por período", err.message);
  ...
  router.get("/mesa", (req, res) => {
    ...
    if (!numero_mesa) {
      return res.status(400).json({ error: "O parâmetro 'numero_mesa' é obrigatório" });
    }
    ...
    if (data_inicio && data_fim) {
       if (!/^\d{4}-\d{2}-\d{2}$/.test(data_inicio) || !/^\d{4}-\d{2}-\d{2}$/.test(data_fim)) {
            return res.status(400).json({ error: "Formato de data inválido para o filtro. Use 'YYYY-MM-DD'" });
       }
    ...
    db.all(sql, params, (err, rows) => {
      if (err) {
        console.error("Erro ao gerar relatório por mesa:", err.message);
  ...
  router.get("/garcom", (req, res) => {
    ...
    if (!nome) {
      return res.status(400).json({ error: "O parâmetro 'nome' (do garçom) é obrigatório" });
    }

    db.get("SELECT id FROM garcons WHERE nome = ?", [nome], (err, garcom) => {
        if (err) {
            console.error("Erro ao verificar o garçom para o relatório:", err.message);
        ...
        db.all(sql, params, (err, rows) => {
          if (err) {
            console.error("Erro ao gerar relatório por garçom:", err.message);
